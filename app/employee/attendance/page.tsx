  ...
  const fetchLoggedInEmployee = async (sessionData: SessionData) => {
    try {
      ...
      if (employeeData) {
        ...
        // Also set as selected employee for history
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6">
          ...
          <div className="bg-gradient-to-r from-violet-600/20 to-purple-600/20 rounded-xl border border-violet-500/30 p-6">
            ...
            <p className="text-slate-300">Track your daily attendance, mark status, and view your attendance history.</p>
          </div>
                ...
                {filteredAttendance.length > 0 ? (
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                              <p className="font-semibold text-white">{record.employeeName}</p>
                              <p className="text-xs text-slate-400">
                                {loggedInEmployee.department} • {loggedInEmployee.position}
                              </p>
                            </div>
                            <span className={`text-xs px-3 py-1 rounded-full border ${getStatusColor(record.status)}`}>
                              {record.status}
                  ...
                  <div className="bg-slate-800 border border-slate-700 rounded-xl p-12 text-center">
                    <div className="text-slate-400">
                      <Users className="h-12 w-12 mx-auto mb-3 text-slate-500" />
                      <p className="font-medium">No attendance records for {currentDate}</p>
                      <p className="text-sm mt-1">Go to "Mark Attendance" tab to mark your attendance</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          {activeTab === 'mark' && (
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-white mb-6">Mark My Attendance</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Form */}
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-violet-900/20 rounded-lg border border-violet-800">
                    <p className="text-sm text-violet-300 flex items-center gap-2">
                      ...
                      Marking attendance for: <span className="font-semibold text-white">{loggedInEmployee.name}</span>
                    </p>
                  ...
                  <button
                    onClick={handleMarkAttendance}
                    className="w-full px-4 py-2.5 bg-violet-600 text-white rounded-lg font-medium text-sm hover:bg-violet-700 transition-colors flex items-center justify-center gap-2"
                  >
                    <CheckCircle2 className="h-4 w-4" />
                    Mark My Attendance
                  </button>
